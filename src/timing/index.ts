...
import type { RoundData, NextRoundInfo, PreviousRoundTiming } from '../state/oredata-store.js';
...
/**
 * Fallback Solana slot duration in milliseconds.
 *
 * **Note:** This is a fallback value only. The SDK fetches the actual network
 * slot duration from the API (`optimized.slotDurationMs`) and uses that for
 * all timing calculations. This constant is only used before the first API
 * response arrives.
 *
 * @see OredataStore.getSlotDurationMs() for the actual dynamic value
 */
export const DEFAULT_SLOT_DURATION_MS = 400;

/**
 * Fallback breather duration in milliseconds (~18s between rounds).
 *
 * **Note:** This is a fallback value only. When `previousRound.endSlot` and
 * `nextRound.startSlot` are both available, the SDK calculates the exact
 * breather duration: `(nextRound.startSlot - previousRound.endSlot) * slotDurationMs`.
 *
 * This fallback is used during the breather when `nextRound.startSlot` is
 * unknown (which is normal—it becomes available when the next round starts).
 *
 * @see getRoundTiming() for the calculation logic
 */
export const DEFAULT_BREATHER_DURATION_MS = 18000;

/**
 * Maximum reasonable breather duration in milliseconds.
 * Used as a sanity cap for corrupted or anomalous slot data.
 */
export const MAX_BREATHER_DURATION_MS = 30000;
...
export interface RoundTiming {
   ...
   * Calculated from nextRoundStartsInMs / breather duration.
   * Use for animations that move right→left during breather.
   */
  breatherProgress: number | null;

  /**
   * Calculated breather duration in milliseconds.
   * When previousRoundEndSlot and nextRound.startSlot are both known,
   * this is the exact duration: (nextRound.startSlot - previousRoundEndSlot) * slotDurationMs.
   * Capped at MAX_BREATHER_DURATION_MS (30000) for sanity.
   * Falls back to DEFAULT_BREATHER_DURATION_MS (18000) when timing data is unavailable.
   */
...
export function getRoundTiming(
  ...
  const {
    ...
    breatherDurationMs: fallbackBreatherDurationMs = DEFAULT_BREATHER_DURATION_MS,
    soonMessage = 'Next round starting soon...',
    idleMessage = 'Waiting for round...',
  } = options;

  const { currentSlot, currentRound, nextRound, previousRoundTiming } = data;

  // Calculate actual breather duration when both end slot and next start slot are known
  let calculatedBreatherDurationMs = fallbackBreatherDurationMs;
  if (previousRoundTiming?.endSlot && nextRound?.startSlot) {
    const breatherSlots = nextRound.startSlot - previousRoundTiming.endSlot;
    if (breatherSlots > 0) {
      calculatedBreatherDurationMs = Math.min(
        breatherSlots * slotDurationMs,
  ...
  if (currentSlot === null || currentRound === null) {
    return {
      ...
      breatherDurationMs: calculatedBreatherDurationMs,
      currentSlot: null,
  ...
  if (nextRoundStartsInMs !== null && nextRoundStartsInMs > 0) {
    breatherProgress = Math.min(1, nextRoundStartsInMs / calculatedBreatherDurationMs);
  }
