...
export function useRoundTiming(options: UseRoundTimingOptions = {}): UseRoundTimingReturn {
  ...
  const { currentSlot, currentRound, nextRound, isReady } = useStore();
  ...
  const timing = useMemo(() => {
    ...
    return getRoundTiming(
      ...
      { slotDurationMs, soonMessage, idleMessage }
    );
  }, [currentSlot, currentRound, nextRound, slotDurationMs, soonMessage, idleMessage]);
...
export { getRoundTiming, formatDuration, DEFAULT_SLOT_DURATION_MS };
