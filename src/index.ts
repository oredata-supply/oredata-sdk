...
export class OredataClient {
   ...
   * @deprecated Use getStore() for data access and createState() for UI timing
   */
   ...
   * state.on('winnerReveal', ({ displayWinner, wasLate }) => {
   *   if (wasLate) {
   *     showQuickReveal(displayWinner);  // Skip spin
   *   } else {
   *     stopWheelOnTile(displayWinner);  // Normal reveal
   *   }
   * });
   *
