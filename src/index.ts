...
import {
  ...
  type ChatEligibilityResponse,
  type ChatMinersStatsResponse,
  type ChatWalletSigner,
  type ChatSendWithWalletRequest,
} from './chat-client.js';
import {
  TokenClient,
...
import {
  ...
  type BidDistributionGlobalSnapshot,
  type RoundData,
  type WinnerData,
  type NextRoundInfo,
...
import {
  SelfServiceClient,
  SelfServiceError,
  type SelfServiceClientOptions,
  type SelfServiceApiKey,
  type SelfServiceApiKeyWithSecret,
  type SelfServiceUser,
  type SelfServiceUsage,
  type SelfServiceUsageHistory,
  type SelfServiceUsageSnapshot,
  type SelfServicePlan,
  type SelfServiceSubscription,
  type SelfServiceSession,
} from './self-service.js';
export type { ConsentTermsResponse, ConsentStatusResponse, ConsentAcceptResponse } from './types.js';
export type {
  RegisterRequest,
  RegisterResponse,
  ListKeysResponse,
...
export { ConsentClient, buildConsentMessage };

// Self-Service Client for bots (wallet-based auth)
export { SelfServiceClient, SelfServiceError };
export type {
  SelfServiceClientOptions,
  SelfServiceApiKey,
  SelfServiceApiKeyWithSecret,
  SelfServiceUser,
  SelfServiceUsage,
  SelfServiceUsageHistory,
  SelfServiceUsageSnapshot,
  SelfServicePlan,
...
export class OredataClient {
  ...
  constructor(options: OredataClientOptions = {}) {
    ...
    this.stateClient.on('snapshot', (snapshot) => {
      ...
      // Extract prices, timing, and treasury data from globals and pass them in optimized format
      ...
      const slotDurationMs = typeof globals.slotDurationMs === 'number' ? globals.slotDurationMs : undefined;
      const motherlodeFormatted = (globals.treasury as Record<string, unknown>)?.motherlodeFormatted;
      ...
      this.store.applyApiResponse({
        data: {
          ...
          optimized: {
            ...
            slotDurationMs,  // Pass through actual network slot duration
            solPriceUsd: typeof solPriceRaw === 'string' ? solPriceRaw : null,
            orePriceUsd: typeof orePriceRaw === 'string' ? orePriceRaw : null,
            motherlodeFormatted: typeof motherlodeFormatted === 'string' ? motherlodeFormatted : null,
          },
        },
  ...
  /**
   * Alias for buildClaimTransaction() - claims SOL rewards.
   * This is the same as buildClaimTransaction() but with a more explicit name.
   * 
   * @example
   * ```typescript
   ...
   * ```
   */
  async buildClaimSolTransaction(request: BuildClaimSolRequest): Promise<TransactionResponse> {
    return this.buildClaimTransaction(request);
  }
  ...
  /**
   * Pause polling (sets mode to 'idle').
   * Use when the browser tab is hidden or user is in settings.
   * State polling stops, but health/quota polling continues at reduced frequency.
   * 
   * @example
   * ```typescript
   ...
   */
  pause(): void {
    this.stateClient.setMode('idle');
  }

  /**
   * Resume polling (sets mode to 'active').
   * Call when the user returns to the game.
   * Immediately fetches fresh state.
   * 
   * @example
   * ```typescript
   * client.resume(); // Immediately starts polling again
   * ```
   */
  resume(): void {
    this.stateClient.setMode('active');
  }

  /**
   * Check if polling is healthy (received data within last 3 seconds).
   * Returns false if data is stale or in idle mode.
   * 
   * @example
   * ```typescript
  ...
  isPollingHealthy(): boolean {
    return !this.stateClient.isDataStale();
  }
  ...
  getLastPollTimestamp(): Date | null {
    const status = this.stateClient.getTransportStatus();
    return status.lastSnapshotAt ? new Date(status.lastSnapshotAt) : null;
  }

  /**
   * Get the current app mode.
   * - 'active': Full-speed polling, all events emitted
   * - 'idle': Reduced polling, for menus/settings
   */
  getMode(): 'active' | 'idle' {
    return this.stateClient.getMode();
  }
  ...
  /**
   * Get the internal StateClient for advanced/developer use cases.
   * 
   * ⚠️ **Internal API** — This is intended for developer tools and dashboards only.
   * Regular apps should use `client.start()`, `client.stop()`, and `client.getStore()`.
   * 
   * @internal
   */
...
export type { MinerClientOptions, MinerClientEvents, MinerStatus, CurrentRoundBids, WalletBidEntry };

// BidTracker exports
export { BidTracker, createBidTracker };
export type { BidTrackerOptions, TrackedBid, WinCheckResult };

// ChatClient exports (ore.supply chat relay)
export { ChatClient, createChatAuth, createChatJwt, ORE_BSM_AUTH_MESSAGE_PREFIX };
export type {
  ChatClientOptions,
  ChatClientEvents,
  ChatMessage,
  ...
  ChatMinersStatsResponse,
  ChatWalletSigner,
  ChatSendWithWalletRequest,
};

// TokenClient exports (ORE token data)
export { TokenClient };
export type {
  TokenClientOptions,
  TokenInfo,
...
// Internal types for developer tools (dashboards, debugging)
// Regular apps should use OredataStore/RoundData instead
export type { StateStoreSnapshot, RoundFrame } from './state/types.js';
export type { AppMode } from './state/state-client.js';
