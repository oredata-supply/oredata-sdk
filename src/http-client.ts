...
export const SDK_VERSION = '0.12.42';
...
export class HttpClient {
  ...
  /**
   * Programmatic registration (beta) using wallet signature.
   * Does not require an existing API key.
   */
  public async registerWithWallet(payload: RegisterRequest): Promise<RegisterResponse> {
    return this.post<RegisterResponse>('/api/v1/register', {
  ...
  /**
   * List API keys for the authenticated owner (requires x-api-key).
   */
  public async listKeys(): Promise<ListKeysResponse> {
    return this.get<ListKeysResponse>('/api/v1/keys');
  }

  /**
   * Create a new API key for the authenticated owner.
   */
  public async createKey(name?: string): Promise<CreateKeyResponse> {
    return this.post<CreateKeyResponse>('/api/v1/keys', { name });
  }

  /**
   * Revoke an API key by id for the authenticated owner.
   */
  public async revokeKey(id: string): Promise<{ success: boolean; error?: string }> {
    const params = new URLSearchParams({ id });
    return this.delete<{ success: boolean; error?: string }>(`/api/v1/keys?${params.toString()}`);
  }
