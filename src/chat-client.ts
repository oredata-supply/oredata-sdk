...
export interface FetchHistoryOptions {
  ...
  limit?: number;
  /** Unix timestamp - get messages before this time (for "load older") */
  before?: number;
  /** Unix timestamp - get messages after this time (for "load newer") */
  after?: number;
  /** Message ID - only messages with id > since */
  since?: number;
...
export interface ChatHistoryResponse {
  ...
  /** Are there more messages available? */
  hasMore?: boolean;
  /** Oldest message timestamp in response */
  oldestTimestamp?: number;
  /** Newest message timestamp in response */
...
export interface ChatSessionResponse {
  ...
  /** If true, user needs to sign to authenticate */
  needsAuth?: boolean;
}
...
export class ChatClient extends EventEmitter<ChatClientEvents> {
  ...
  /** Cached sessions by wallet address */
  private sessionCache: Map<string, { jwt: string; expiresAt: number }> = new Map();
   ...
   *
   * Supports pagination for loading older/newer messages.
   *
   * @example
   * ```typescript
   ...
   *     limit: 50,
   *     before: messages[messages.length - 1].ts,
   *   });
   * }
   *
   * // Load newer messages (after reconnect)
   * const newer = await chat.fetchHistory({
   *   after: lastKnownTimestamp,
   * });
   * ```
   */
  async fetchHistory(options: FetchHistoryOptions = {}): Promise<ChatHistoryResponse> {
    const params = new URLSearchParams();
    if (options.limit) {
  ...
  /**
   * Fetch chat history via REST endpoint (legacy, returns messages only).
   * @deprecated Use `fetchHistory()` for pagination support.
   */
  async getHistory(options: { limit?: number; since?: number } = {}): Promise<ChatMessage[]> {
    const result = await this.fetchHistory(options);
  ...
  async send(text: string, wallet: ChatWalletSigner): Promise<ChatSendResponse> {
    ...
    const localCached = this.sessionCache.get(authority);
    if (localCached && localCached.expiresAt > now + 60) {
      return this.sendMessage({ text, jwt: localCached.jwt });
    }

    // 2. Check API cache (no signature required)
    const apiCached = await this.getSession(authority);
    if (apiCached.success && apiCached.jwt && apiCached.expiresAt && apiCached.expiresAt > now + 60) {
      // Update local cache with API's cached session
      this.sessionCache.set(authority, {
        jwt: apiCached.jwt,
        expiresAt: apiCached.expiresAt,
      });
      return this.sendMessage({ text, jwt: apiCached.jwt });
    }

    // 3. Need to create new session (wallet will sign)
  ...
  /**
   * Get cached session from API (no signature required).
   * Returns cached JWT if API has one, otherwise indicates auth is needed.
   */
  async getSession(authority: string): Promise<ChatSessionResponse> {
    const url = `${this.apiBaseUrl}/chat/session/${encodeURIComponent(authority)}`;
    ...
    try {
      ...
      // If API call fails, just indicate auth is needed
      return { success: false, needsAuth: true };
    }
  }

  /**
   * Clear cached session for a wallet (forces re-authentication on next send)
   */
  clearSession(authority: string): void {
