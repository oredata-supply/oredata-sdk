...
const MODE_INTERVALS = {
  active: {
    ...
    health: 5_000,   // 5s health polling
...
export interface StateEngineOptions {
  ...
  /** 
   * Enable quota/billing monitoring. Defaults to true.
   * Set to false if you don't need quota tracking to reduce polling.
...
export type StateClientEvents = {
  ...
  winner: (payload: { roundId: string; winner: number | null; type: 'optimistic' | 'final' }) => void;
...
const HEALTH_POLL_INTERVALS = {
  normal: 5_000,
  recovery: 2_000, // Poll faster when trying to detect recovery
};
...
export class StateClient extends EventEmitter<StateClientEvents> {
  ...
  constructor(
    ...
    this.healthPollIntervalMs = Math.max(5_000, options.healthPollIntervalMs ?? 5_000);
    this.quotaPollingEnabled = options.quotaPolling !== false; // Enabled by default
  ...
  private checkBillingWarningsFromQuota(snapshot: BillingSnapshot): void {
    const { liveRequests, historicalQueries, daysRemaining, forecast } = snapshot;
