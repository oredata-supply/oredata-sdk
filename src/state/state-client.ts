...
export class StateClient extends EventEmitter<StateClientEvents> {
  ...
  private unreachableSince: number | null = null; // When we became unreachable
  ...
  getConnectionState(): ConnectionState {
    // Calculate downtime if we just recovered from unreachable
    let downtimeMs: number | null = null;
    if (
      this.previousConnectionStatus === 'unreachable' &&
      this.connectionStatus === 'connected' &&
      this.unreachableSince !== null
    ) {
      downtimeMs = Date.now() - this.unreachableSince;
    }
      ...
      if (stateRecentlySucceeded && this.connectionStatus !== 'unreachable') {
        ...
        try {
          ...
          if (
            ...
            this.setConnectionStatus('unreachable');
  ...
  /**
   * Update connection status with proper tracking of previous state.
   */
  private setConnectionStatus(newStatus: ConnectionStatus): void {
    if (newStatus === this.connectionStatus) return;

    // Track previous status
    this.previousConnectionStatus = this.connectionStatus;

    // Track when we became unreachable (for downtime calculation)
    ...
    // Clear unreachableSince when we recover (downtime will be calculated in getConnectionState)
    if (newStatus === 'connected' && this.previousConnectionStatus === 'unreachable') {
      // Keep unreachableSince until after we emit, so getConnectionState can calculate downtimeMs
      setTimeout(() => {
        this.unreachableSince = null;
      }, 0);
    }
