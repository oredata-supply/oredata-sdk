...
export class StateStore {
  ...
  applySnapshot(snapshot: StateV3Response): void {
    ...
    // Check current round for winners
    if (this.currentRoundId) {
      this.tryEmitWinner(this.currentRoundId);
    }
    
    // Check if we should also fire winner for just-finalized round
    // Only do this if current round hasn't started collecting bids yet
    // (i.e., we're still in the result reveal window)
    const finalizedId = this.latestFinalizedRoundId;
    if (finalizedId && finalizedId !== this.currentRoundId) {
      const currentFrame = this.currentRoundId 
        ? this.frames.get(this.currentRoundId) 
        : null;
      
      // Current round is "active" if it has slot data or bids
      const currentRoundHasBids = currentFrame?.liveSlot !== null || 
        parseFloat(currentFrame?.liveData?.totals?.deployedSol ?? '0') > 0;
      
      if (!currentRoundHasBids) {
        // Still in transition - fire previous winner
        this.tryEmitWinner(finalizedId);
      }
    }
  ...
  private applyPhase(metadata: RoundPhasePayload | null): void {
    ...
    // Only emit phaseChange when the actual phase changes (BETTING → SPINNING → RESULT)
    // NOT when timing metadata (phaseSince/phaseUntil) updates - those change every poll
    const phaseChanged = !this.phase || this.phase.phase !== nextPhase.phase;
    this.phase = nextPhase;
    if (phaseChanged && this.shouldEmitRoundEvents()) {
