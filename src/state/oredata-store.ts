...
export class OredataStore extends EventEmitter<OredataStoreEvents> {
  ...
  applyApiResponse(response: StateV3Response): void {
    ...
    for (const frame of frames) {
      this.processFrame(frame as RoundFrame, now);
    }
    ...
    if (currentFrame) {
      this.processFrame(currentFrame as RoundFrame, now);
    }

    // Check for finalized winner
    if (newFinalizedRoundId) {
      this.checkForWinner(newFinalizedRoundId, now);
    }
  ...
  private processFrame(frame: RoundFrame, now: number): void {
  ...
  private checkForWinner(roundId: string, now: number): void {
    const round = this.rounds.get(roundId);
