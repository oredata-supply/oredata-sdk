...
export interface WinnerDisplay {
  roundId: string;
  tile: number; // 0-indexed
  displayTile: number; // 1-indexed for UI
  revealedAt: number;
  wasLate: boolean;
  arrivalMs?: number; // ms since round ended when data arrived (may not be set for old reveal events)
}
...
export interface WinnerRevealPayload {
  ...
  displayWinner: number; // 1-indexed
  wasLate: boolean;
...
export class OredataState extends EventEmitter<OredataStateEvents> {
  ...
  private revealWinner(roundId: string, tile: number, wasLate: boolean, arrivalMs: number): void {
    ...
    this.displayedWinner = {
      roundId,
      tile,
      displayTile: tile + 1, // 1-indexed for UI
      revealedAt: now,
      wasLate,
      arrivalMs,
    };
