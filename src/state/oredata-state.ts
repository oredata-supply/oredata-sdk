...
export class OredataState extends EventEmitter<OredataStateEvents> {
  ...
  private handleMiningStatusChanged: (payload: MiningStatusChangedPayload) => void;

  // Track last emitted phase+round to avoid duplicate guards blocking new round events
  private lastEmittedPhaseRound: string | null = null;
  ...
  constructor(store: OredataStore, config: OredataStateConfig = {}) {
    ...
    this.handleMiningStatusChanged = this.onMiningStatusChanged.bind(this);

    // Subscribe to store events
    this.store.on('roundCompleted', this.handleRoundCompleted);
    this.store.on('roundStarted', this.handleRoundStarted);
    this.store.on('miningStatusChanged', this.handleMiningStatusChanged);
  }
  ...
  /**
   * Handle mining status changes from OredataStore.
   * This allows us to transition to SPINNING when the round ENDS,
   * not just when the winner arrives.
   */
  private onMiningStatusChanged(payload: MiningStatusChangedPayload): void {
    const { roundId, status, previousStatus } = payload;

    // Only handle transition from ACTIVE to EXPIRED (round ended)
    if (previousStatus === 'ACTIVE' && status === 'EXPIRED') {
      // If we're still in BETTING, transition to SPINNING (waiting for winner)
      if (this.currentDisplayPhase === 'BETTING' && this.config.showSpinPhase) {
  ...
  private setDisplayPhase(phase: DisplayPhase, roundId: string | null): void {
    // Build a key combining phase and roundId
    // This allows BETTING for round N and BETTING for round N+1 to be different events
    const phaseRoundKey = `${phase}:${roundId ?? 'null'}`;
    
    // Skip only if BOTH phase AND round are the same
    if (phase === this.currentDisplayPhase && phaseRoundKey === this.lastEmittedPhaseRound) {
      return;
    }

    const previousPhase = this.currentDisplayPhase;
    this.currentDisplayPhase = phase;
    this.lastEmittedPhaseRound = phaseRoundKey;
  ...
  stop(): void {
    ...
    this.store.off('miningStatusChanged', this.handleMiningStatusChanged);
