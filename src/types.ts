...
// RoundFrameKeyMeta and SerializedInstruction removed in v0.12.0
// These were only used by the old buildBid()/buildClaim() methods
// Use buildBidTransaction()/buildClaimTransaction() instead
...
export interface BuildBidRequest {
  ...
  roundId?: string;
  /** 
   * Skip server-side transaction simulation. 
   * Default: false (simulation enabled).
   * Set to true for lower latency, but Phantom may show warnings if tx would fail.
   */
  skipSimulation?: boolean;
}

/** Request to build a SOL claim transaction */
...
/** Alias for BuildClaimRequest - for discoverability */
export type BuildClaimSolRequest = BuildClaimRequest;

/** Request to build an ORE token claim transaction */
export interface BuildClaimOreRequest {
  authority: string;
  /** 
   * Skip server-side transaction simulation. 
   * Default: false (simulation enabled).
...
// BuilderMetadata and BuildInstructionsResponse removed in v0.12.0
// These were only used by the old buildBid()/buildClaim() methods
// Use buildBidTransaction()/buildClaimTransaction() which return TransactionResponse
...
export interface TransactionResponse {
  ...
  metadata: {
    ...
    /** True when checkpoint instruction was added for this transaction */
...
export type ConsentStatusState = 'missing' | 'outdated' | 'accepted';

export interface ConsentStatusResponse {
  status: ConsentStatusState;
...
export interface ConnectionState {
  ...
  failureCount: number;
  lastError: string | null;
  /** Previous status before the current one (null on initial state) */
  previousStatus: ConnectionStatus | null;
  /** Milliseconds spent in previous 'unreachable' state before recovery (null if not recovering) */
...
export interface CreateKeyResponse {
  key: ApiKeySummary & { api_key: string };
}
