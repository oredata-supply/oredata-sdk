...
- **Chat Send Support in `ChatClient`** — Send messages to the ore.supply community chat
  ...
  Now you can send messages from any OreData-powered frontend that appear in the unified ore.supply chat:
  ...
  // 1. Check eligibility (must have mined in last 30 days)
  const eligibility = await chat.isEligible(wallet.publicKey.toBase58());
  if (!eligibility.eligible) {
    console.log('Mine in any round to unlock chat!');
    return;
  }

  // 2. Create a session (wallet signs a message)
  ...
  // 3. Send messages (using cached JWT)
  const result = await chat.sendMessage({
    text: 'Hello from gmore.fun!',
    jwt: session.jwt!,
  });
  ...
  **Features:**
  ...
  - **Session caching** — JWT cached for 24 hours (no re-signing needed)

- **New ChatClient methods:**
  - `sendMessage(request)` — Send a chat message
  - `createSession(request)` — Create a JWT session from wallet signature
