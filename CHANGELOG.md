...
- **Chat JWT creation now works correctly** — Fixed critical bug where JWT signatures didn't match ore.supply's expected format

  The JWT signature must be an Ed25519 signature of `header.payload` (the JWT content itself). Previous versions signed an arbitrary message, which ore.supply rejected.

### Added

- **`createChatJwt(wallet)` helper** — Creates properly formatted JWTs for ore.supply chat
...
- **`chat.send(text, wallet)` simplified method** — One-liner for sending messages

  ```typescript
  // Super simple! Creates JWT automatically
  const result = await chat.send('Hello everyone!', wallet);
  ```

- **`ChatWalletSigner` type** — Compatible with @solana/wallet-adapter wallets

### Changed

- **Recommended approach**: Use `chat.send(text, wallet)` or `createChatJwt(wallet)` instead of `createSession()`
- `createSession()` is now **deprecated** (signature format was incorrect)
