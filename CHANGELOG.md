...
- **Timing Deep Dive documentation** — Comprehensive guide for building accurate progress bars and countdowns, based on feedback from the ORE Pump integration team

  New documentation includes:
  - ASCII diagrams showing BETTING/BREATHER two-phase timing model
  - Progress bar direction table (`progress` 0→1 vs `breatherProgress` 1→0)
  - Dynamic vs Fallback values explanation (constants are fallbacks, not primary)
  - **Data Availability Timeline** — Critical diagram explaining why `nextRound.startSlot` is unknown during breather
  - Troubleshooting section for common timing issues

- **Improved `useRoundTiming()` documentation** — Return values now grouped by phase (BETTING/BREATHER/Universal) with warning callout about estimation during breather

- **"Animated Progress Fuse" recipe** — Complete example with correct animation directions

### Changed

- **JSDoc comments improved** in `useRoundTiming.ts` and `timing/index.ts`:
  - Constants now clearly marked as "Fallbacks Only"
  - Added `@see` references to dynamic value sources
  - Comprehensive parameter and return documentation

### Notes

- No SDK source code behavior changes in this release (documentation only)
- Addresses integration pain points documented by ORE Pump team
...
- **Changelog backfill** — Added missing entries for 0.16.0 and 0.16.1
...
- **SDK README restructured** — Website-first approach with prominent oredata.supply CTA for npm visitors
- **Simplified Quick Start** — Removed unnecessary `baseUrls` parameter (uses default `api.oredata.supply`)
- **Documentation links updated** — All paths changed from `/cookbook` to `/examples`
...
- **Major documentation restructure** — New reference documentation for all SDK clients:
  - `docs/reference/OREDATA-CLIENT.md` — Core client reference
  - `docs/reference/MINER-CLIENT.md` — Wallet tracking reference
  - `docs/reference/CHAT-CLIENT.md` — Chat integration reference
  - `docs/reference/TOKEN-CLIENT.md` — ORE token data reference
  - `docs/reference/CONSENT-CLIENT.md` — Terms & consent reference
  - `docs/reference/STATE.md` — State management reference
  - `docs/reference/ERRORS.md` — Error handling reference

- **Self-service billing documentation expanded** — Complete subscription flow with USDC payment examples in `docs/reference/SELF-SERVICE.md`

- **Fee economics focus** — README highlights up to 90% discounts and transparent pricing

### Notes

- Version 0.15.0 was skipped intentionally
- No SDK source code changes in this release (documentation only)
...
- **Usage Snapshots** — Get hourly API usage data for time-series charts

  ```typescript
  const client = new SelfServiceClient();
  await client.auth.login({ wallet, message, signature });

  // Get last 7 days of hourly usage (default)
  const { snapshots } = await client.usage.getSnapshots();

  // Get last 30 days
  const { snapshots } = await client.usage.getSnapshots({ hours: 720 });

  // Chart the data
  snapshots.forEach(s => {
    console.log(`${s.hour}: ${s.liveRequests} live, ${s.historicalQueries} historical`);
  });
  ```

- **Project Usage Snapshots** — Track usage per project
...
- **New types:**
  - `SelfServiceUsageSnapshot` — Hourly usage data point with `hour`, `liveRequests`, `historicalQueries`

## [0.13.0] - 2025-12-21

### Added

- **`SelfServiceClient.projects` namespace** — Full project, terms, and consent management for bots

  ```typescript
  import { SelfServiceClient } from '@oredata/sdk';

  const client = new SelfServiceClient();
  await client.auth.register({ wallet, message, signature });

  // Projects
  const { project } = await client.projects.create({ name: 'My Bot', slug: 'my-bot' });
  const { projects } = await client.projects.list();
  const { project: details } = await client.projects.get(projectId);  // includes stats
  await client.projects.update(projectId, { name: 'Updated Name' });
...
- **New types exported:**
  - `SelfServiceProject` — Project data
  - `SelfServiceProjectWithStats` — Project with activeTerms and stats
  - `SelfServiceTerms` — Terms version data
  - `SelfServiceConsent` — Consent record
  - `SelfServicePagination` — Pagination metadata

- **`SelfServiceClient.keys.assign(keyId, projectId)`** — Assign an API key to a project for consent tracking

### Changed

- `SelfServiceClient` now includes all CRUD operations for projects, terms, and consents
- Bot developers can now fully self-service T&C requirements programmatically

## [0.12.46] - 2025-12-20

### Added

- **Accurate `breatherDurationMs` calculation** — Now calculated from actual on-chain slot data

  When `previousRound.endSlot` and `nextRound.startSlot` are both known, the SDK calculates the exact breather duration:

  ```tsx
  const { breatherDurationMs, breatherProgress, inBreather } = useRoundTiming();

  // breatherDurationMs is now precise when timing data is available
  // Falls back to 18000ms (DEFAULT_BREATHER_DURATION_MS) when unavailable
  console.log(`Breather duration: ${breatherDurationMs}ms`);
  ```

  Previously, breather animations used a hardcoded 18s estimate. Now the SDK uses real slot data from the API for accurate progress calculation.

- **`previousRound.endSlot` in API response** — The optimized state response now includes the previous round's end slot for timing calculations

- **`PreviousRoundTiming` type** — New type exported from SDK for previous round timing info

### Changed

- **`breatherProgress` now uses calculated duration** — Progress is calculated using the actual breather duration when available, resulting in accurate animation timing

- **Sanity check on calculated duration** — `breatherDurationMs` is capped at `MAX_BREATHER_DURATION_MS` (30000ms) to prevent animation issues from corrupted slot data
