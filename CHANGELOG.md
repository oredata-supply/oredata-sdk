...
- **bm: sustainable platform fees** — Platform fees now scale dynamically during the last 10 seconds of each round:
  - Base rate: 0.25% (unchanged for bids >10s before round end)
  - Surge rate: 0.25% → 2.50% (discrete 1-second steps in final 10s)
  - The `platformFeeRate` field in bid responses now reflects the actual rate charged
