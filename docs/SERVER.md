...
> ⚠️ **Important:** The multiplexer is designed for **SSE streaming to browsers**. For reliable winner detection on your backend, use `OredataClient` + `store.on('roundCompleted')` instead. See [ARCHITECTURE.md](./ARCHITECTURE.md).
...
|----------|---------------------|
| **Stream bid data to browsers** | ✅ `createMultiplexer` + `expressSSE` |
| **Detect winners on backend** | ❌ Use `OredataClient` + `store.on('roundCompleted')` |
| **Game logic / phase detection** | ❌ Use `OredataClient` + Layer 1 events |

The multiplexer is a thin wrapper for efficient broadcasting. For game logic, use the full `OredataClient`.
