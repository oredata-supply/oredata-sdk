    ...
    "frames": [
      {
        "roundId": "12345",
        "liveData": {
          "observedAt": "2025-12-15T17:44:30.000Z",
          "roundId": "12345",
          "mining": {
            "startSlot": "382009500",
            "endSlot": "382010100",
          ...
          "perSquare": {
            "counts": ["0","1","0"],
            "deployedSol": ["0.00","0.42","0.00"]
          },
          "bidDistributionGlobal": {
            "updatedAt": 1734412345678,
            "sampleSize": 912,
            "source": "estimate",
            "estimateRoundsUsed": 7,
            "p50Lamports": "1200000",
            "p80Lamports": "8900000",
            "p90Lamports": "15000000",
            "p95Lamports": "25000000",
            "p99Lamports": "42000000",
            "avgLamports": "5500000"
          }
        },
...
**Platform Fees (SSOT):**
- `optimized.platformFeeRate` — Current fee rate (0.0025-0.03). Display this to users before bidding.
  - 0.0025 (0.25%) when >15s remaining
  - Up to 0.03 (3.0%) in the last 2.5 seconds
  - Same value is used by `buildBidTransaction()` — no client calculation needed
- `optimized.platformFeeMinUsd` — Minimum fee in USD ($0.01). Always charged, even on small bids.

**Phase Timing (Progress Animations):**
- `optimized.phaseDurations.spinningMs` — Expected winner reveal time (~15s, varies)
- `optimized.phaseDurations.resultMs` — Time showing winner (8s)
- `optimized.phaseDurations.breatherMs` — Gap between rounds (5s)

**Note:** SPINNING phase duration is unpredictable (depends on oracle). Use for estimates only.

**Winner Delay:** When `meta.winnerRevealIn > 0`, winner info is temporarily hidden based on your plan.

#### `GET /v3/rounds/:roundId/bid-distribution/tiles`

Per-tile bid size percentiles for a round (live-only).
  ...
  "tiles": [
    {
      ...
      "p99Lamports": "42000000",
      "avgLamports": "5500000",
      "tileDeployedLamports": "4500000000",
      "otherTilesDeployedLamports": "98500000000"
    }
  ]
}
```
...
**Fee Fields:**
- `platformFeeLamports` — Actual fee charged in lamports
- `platformFeeRate` — Rate applied (0.0025-0.03)
- `platformFeeMinUsd` — Minimum fee floor ($0.01)
...
```json
{
  ...
  "registerApiUrl": "https://api.oredata.supply/api/v1/register",
...
### Self-Serve Registration & Keys (beta)

**Endpoints (now served by the core API):**
- `POST /api/v1/register` — Register with a Solana wallet signature, returns an API key (free plan).  
- `GET /api/v1/keys` — List keys for the authenticated owner (requires `x-api-key`).  
- `POST /api/v1/keys` — Create a new key for the authenticated owner.  
- `DELETE /api/v1/keys?id=<keyId>` — Revoke a key.  

**SDK helpers (v0.12.42+):**
```ts
// Registration (no API key required)
await client.http.registerWithWallet({
  walletAddress: '7q2K...',
  message: 'oredata.supply registration: <nonce>',
  signature: '<base58 signature>',
});

// Key management (requires x-api-key)
await client.http.listKeys();
await client.http.createKey('Production Backend');
await client.http.revokeKey('<key-id>');
```

Notes: DB must be configured; endpoint returns 503 if unavailable. Rate limits apply per plan.
