...
const client = new OredataClient({
  ...
  apiKey: process.env.OREDATA_API_KEY, // Get via API (POST /api/v1/register) or oredata.supply/register
  pollIntervalMs: 1000,
});
...
## 2b. Token Prices (Optional)

Get real-time SOL and ORE prices in USD:

```typescript
// After client.start(), prices are updated on each poll
...
if (round && solPrice) {
  ...
  console.log(`💵 Pot value: $${potUsd.toFixed(2)}`);
}
```
...
- API (beta): `POST https://api.oredata.supply/api/v1/register` with wallet signature
...
| `GET /v3/rounds/:roundId/bid-distribution/tiles` | Per-tile bid size percentiles (live-only) |
