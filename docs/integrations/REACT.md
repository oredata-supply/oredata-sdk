...
function GameTimer() {
  const {
    ...
    progress,          // 0-1 for active round (left→right)
    breatherProgress,  // 1-0 for breather (right→left)
  ...
  return (
    <div className={inRound ? 'betting' : 'breather'}>
      ...
      <span>{countdown}</span>
      {/* Active round: progress bar left→right */}
      {inRound && progress !== null && (
        <div className="progress-bar">
          <div style={{ width: `${progress * 100}%` }} />
        </div>
      )}
      {/* Breather: progress bar right→left */}
...
const timing = useRoundTiming({
  ...
  breatherDurationMs: 18000, // Estimated breather duration for progress calc
});
```
...
| `progress` | number \| null | 0-1 for progress bars (active round only) |
| `breatherProgress` | number \| null | 1→0 during breather (for right→left animations) |
| `breatherDurationMs` | number | Calculated breather duration (falls back to 18000ms) |
