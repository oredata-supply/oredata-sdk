...
│  │  • Always adds checkpoint for existing miners                 │  │
│  │  • Builds bid instructions                                    │  │
│  │  • Adds platform fee (0.25%-3% surge pricing)                 │  │
...
interface BidTransactionResponse {
  ...
  platformFeeLamports: number;   // Our fee (0.25%-3% surge pricing)
  platformFeeRate: number;       // Actual rate charged (0.0025-0.03)
  platformFeeMinUsd: number;     // Minimum fee floor ($0.01)
  metadata: {
    roundId: string;             // Current round
    tiles: number[];             // Tiles in this bet
    checkpointIncluded: boolean; // True when checkpoint instruction added
    needsCheckpoint?: boolean;   // Legacy alias of checkpointIncluded
  };
}
```

Note: If cached miner data is inconsistent, the API refreshes miner state from RPC once; `metadata.checkpointIncluded` is the source of truth and may remain `false` if the round id cannot be determined (clients can retry).
...
```
1. Checkpoint (existing miners) — Settle previous round (no-op if already settled)
2. Deploy (bid)                — Main ORE program interaction
3. Platform fee transfer       — API relay fee (0.25%-3% surge pricing)
```
...
Platform fees scale dynamically based on time remaining in the round:

| Time Remaining | Fee Rate |
|----------------|----------|
| > 15 seconds   | 0.25%    |
| 15s - 12.5s    | 0.50%    |
| 12.5s - 10s    | 1.00%    |
...
**Why surge pricing?** Last-second bids require time-critical relay with premium 
RPC priority. Higher fees during peak demand ensure sustainable infrastructure.

### Minimum Fee

All bids incur at least **$0.01 USD** in fees, calculated dynamically from the current SOL price. This ensures sustainable transaction processing even for small bids.

| SOL Price | Minimum Fee (lamports) | Minimum Fee (SOL) |
|-----------|------------------------|-------------------|
| $100      | 100,000                | 0.0001            |
| $200      | 50,000                 | 0.00005           |
| $300      | 33,333                 | 0.000033          |

The minimum kicks in when the percentage-based fee would be less than $0.01. For example:
- Bid 0.001 SOL at 0.25% = 0.0000025 SOL (~$0.0006) → **minimum applies**
- Bid 0.1 SOL at 0.25% = 0.00025 SOL (~$0.06) → **percentage applies**

The response includes `platformFeeRate` so you can display the actual rate charged:

```typescript
const { platformFeeLamports, platformFeeRate } = await client.buildBidTransaction({...});
console.log(`Fee: ${platformFeeLamports / 1e9} SOL (${platformFeeRate * 100}%)`);
```

You can also get the current fee rate from `OredataStore` without building a transaction:
...
| Checkpoint instruction | API (for existing miners) |
| Platform fee transfer | API (0.25%-3% for bids, 0% for claims) |
