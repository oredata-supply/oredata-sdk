...
│  │  • Adds platform fee (0.25%-2.5% surge pricing)              │  │
...
interface BidTransactionResponse {
  ...
  platformFeeLamports: number;   // Our fee (0.25%-2.5% surge pricing)
  platformFeeRate: number;       // Actual rate charged (0.0025-0.025)
  metadata: {
...
3. Platform fee transfer       — API relay fee (0.25%-2.5% surge pricing)
```
...
| Platform fee transfer | API (0.25%-2.5% for bids, 0% for claims) |
