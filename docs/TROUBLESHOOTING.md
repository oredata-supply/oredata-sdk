...
**Note:** The legacy `buildBid()` method was removed in v0.12.0. Use `buildBidTransaction()` instead.
...
|---------|-------|----------|
| `source: "ip"` | No API key, hitting IP limits | Get an API key: UI at [oredata.supply/register](https://oredata.supply/register) or API `POST https://api.oredata.supply/api/v1/register` (wallet-sign) |
...
```typescript
// Connection monitoring - check isConnected from store
const store = client.getStore();

// The SDK auto-reconnects with backoff
// Monitor via useStore() in React or store events
```
...
```bash
# Test connectivity
curl -v https://api.oredata.supply/v3/health
```
...
useEffect(() => {
  console.log('State changed:', { currentRound, isConnected });
}, [currentRound, isConnected]);
```
...
## Timing & Progress Bar Issues

### Progress bar jumps or snaps at phase transitions

**Symptom:** Progress bar suddenly jumps when the breather ends and new round starts.

**Cause:** During the breather, `breatherProgress` is **estimated** because `nextRound.startSlot` is unknown until the next round starts. When the new round starts, the UI updates to the correct state.

**This is expected behavior.** For smoother UX, consider:

```tsx
const { breatherProgress, nextRoundKnown } = useRoundTiming();

// Option 1: Add a brief transition animation on phase change
<div className="transition-all duration-300" style={{ width: `${breatherProgress * 100}%` }} />

// Option 2: Show an "estimated" indicator when timing is approximate
{!nextRoundKnown && <span className="text-gray-400">~</span>}
```

See [Timing Deep Dive](./01-CORE-CONCEPTS.md#timing-deep-dive) for details on why this happens.

---

### Progress bar animates in wrong direction

**Symptom:** Bar shrinks when it should fill, or fills when it should shrink.

**Cause:** Using `progress` during breather or `breatherProgress` during betting.

**Fix:** Use the correct progress value for each phase:

```tsx
const { progress, breatherProgress, inRound, inBreather } = useRoundTiming();

if (inRound) {
  // BETTING: use `progress` (0→1, fills left to right)
  return <div style={{ width: `${progress * 100}%` }} />;
}

if (inBreather) {
  // BREATHER: use `breatherProgress` (1→0, shrinks right to left)
  return <div style={{ width: `${breatherProgress * 100}%` }} />;
}
```

| Phase | Value | Direction |
|-------|-------|-----------|
| BETTING | `progress` | 0→1 (fills left→right) |
| BREATHER | `breatherProgress` | 1→0 (shrinks right→left) |

---

### Hardcoded timing doesn't match actual rounds

**Symptom:** Using constant values (400ms slot, 18s breather) but timing is slightly off.

**Cause:** Constants are fallbacks. The SDK uses **dynamic values from the network**.

**Fix:** Always use `useRoundTiming()` values instead of hardcoding:

```tsx
// DON'T hardcode:
const SLOT_DURATION = 400;
const BREATHER_DURATION = 18000;

// DO use SDK values:
const { breatherDurationMs } = useRoundTiming();
// breatherDurationMs is calculated from actual slot data when available
```

The SDK:
1. Fetches actual network slot duration from the API
2. Calculates exact breather duration from `(nextRound.startSlot - previousRound.endSlot) * slotDurationMs`
3. Falls back to constants only before first API response or when slot data unavailable
