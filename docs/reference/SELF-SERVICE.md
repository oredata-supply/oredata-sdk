...
### Subscribe to a Paid Plan (Complete Flow)

Subscribing to a paid plan requires a USDC payment on Solana. Here's the complete flow:
...
async function subscribeToPlan(
  ...
  // 1. Request subscription - API tells us if payment needed
  ...
  // 2. Payment required - build USDC transfer
  const { payment } = result;
  console.log(`Payment required: ${payment.amountUsdc} USDC`);

  const connection = new Connection('https://api.mainnet-beta.solana.com');
  const usdcMint = new PublicKey(payment.usdcMint);
  const treasuryWallet = new PublicKey(payment.treasuryWallet);
  ...
  // USDC has 6 decimals
  const amountLamports = BigInt(Math.round(payment.amountUsdc * 1_000_000));
  ...
  // 5. Confirm subscription with API
  const confirmation = await client.billing.confirm({
    planId,
    billingPeriod,
    txSignature,
    payerWallet: walletAddress,
  ...
  console.log('Subscription confirmed!', confirmation.subscription);
...
- Your wallet must have USDC in its Associated Token Account
- The API validates the payment on-chain before activating your subscription

### Billing Methods Reference
...
// Start subscription (see complete flow above)
const result = await client.billing.subscribe({
  planId: 'pro',
